...
Enabling incremental pod installation will also be gated by an installation option `use_incremental_installation` that depends on `generate_multiple_pod_projects` also being enabled. Installation will raise an exception early on if this condition isn't met.

In addition to the installation option, we will also provide a new installation flag `--force-full-install` that can be used to ignore the contents of the cache and force a complete installation.
...
The cache will exist under the `.project_cache` dir and store two files for the cases listed above: `target_key_cache` and `target_metadata_cache`

#### Key Cache: `./project_cache/target_key_cache`
##### `TargetCacheKey`
...
*(Maybe include?)Note on storing sets of files:* There are a couple ways we could go about storing the set of files: create a unique checksum from the list of files, or directly store the set of files as an array. Storing the list of files as an array directly seems to be the better option since it allows us to output the list of files causing a project to be regenerated that can be used by the `--verbose` flag and local testing. In addition, we will mostly be checking `TargetKeyCache` objects generated from a `PodTarget` object against the equivalent target parsed from the cache; thus, we will already have to perform a linear operation (as opposed to a constant) in order to compute the checksum from the list of files on the `PodTarget` object to compare against the cached checksum. As a result, storing the set of files as an array seems to be the better option with only one extra iteration incurred for performance.
...
class TargetKeyCache
	...
	# Hash representation of TargetKeyCache. Used for cache storage.
	def to_hash

	# Converts hash to TargetKeyCache.
	def self.cache_key_from_hash(hash)

	# Converts #pod_target type to TargetKeyCache.
	def self.cache_key_from_pod_target(pod_target)

	# Converts #aggregate_target type to TargetKeyCache.
	def self.cache_key_from_aggregate_target(aggregate_target)
```

##### `ProjectKeyCache`
The `ProjectKeyCache` is responsible for creating an in-memory representation of the cache stored in `.project_cache/target_key_cache`. In addition to storing the properties of each `TargetKeyCache` per target, this will also store the hash of user build configurations and a cache version. Since the user build configurations is applied to all projects, any changes to that hash would require a full installation. The cache version is stored for backwards compatibility if changes are made down to the structure of the cache that requires flushing its contents.
...
class ProjectKeyCache
	...
	# Parses contents of file and creates ProjectKeyCache instance.
	def self.from_file(path)
```

#### Metadata Cache: `.project_cache/target_metadata_cache`
...
_Note: A future optimization could involve only opening up the project and selectively updating the properties that have changed. This would go along with updating the `TargetKeyCache` key_difference method to return more symbols (i.e. `:build_settings` or `:target_dependencies`)_
...
Similar to `ProjectKeyCache`, this object is responsible for creating an in-memory representation of the cache stored in `.project_cache/target_metadata_cache`.
...
In the pipeline of installation steps, the `PodProjectCacheAnalyzer` will run right before project generation such that the project generator will only inject in the list of targets that the analyzer has determined needs to be generated.
...
`def add_cached_dependency` will be added to the `Xcodeproj::Project::Object::AbstractTarget` object reopenend and extended inside of CocoaPods. This will use the metadata `native_target_uuid` and `container_project_uuid` in order to recreate a target dependency.


## Backwards Compatibility

The existing API on the `Installer` includes two properties:`pods_project` and `pod_target_subprojects`. With incremental installation, these projects may or may not be generated. As a result, we should add to the documentation that these values _can_ not exist. Instead of using those properties, post-install hooks should use a new property called `changed_projects` which is just an array of the list of projects that were generated as a result of the incremental installation.

## Other Pod Commands that use `installer.rb`

`pod update` and `pod analyze` should remain unchanged and won't need any changes as a result of the caching.
